...
class SimpleFCFSProcess:
    ...
    def __lt__(self, other) -> bool:
        if not isinstance(other, SimpleFCFSProcess):
            raise TypeError(f'Object of type SimpleFCFSProcess must be passed. but {type(other).__name__} was passed')
...
class SimpleFCFS:
    def __init__(
                    ...
                    randomGeneratorFunc: Callable = lambda n: getUniformRandomNumber(
                                                                in_range=10**5,
                                                                shape=(n, 2)
                                                            )
                ) -> None:
    ...
    def __runTest(self) -> tuple:
        ...
        while (not self.processes_queue.isEmpty()) or (not self.ready_queue.isEmpty()):
            ...
            current_process = self.ready_queue.dequeue(); current_process: SimpleFCFSProcess
    ...
    def addNewArrivedProcessesToReadyQueue(self, timer: int):
        next_process = self.processes_queue.next(); next_process: SimpleFCFSProcess
    ...
    def addProcesses(self, randoms):
        for num in randoms:
            self.processes_queue.enqueue(item=SimpleFCFSProcess(*num))
